...
{
    Point(){};
    Point(int numi,int plai){num=numi;pla=plai;}
    int num,pla;
...
int main()
{
	...
	{
		sto[++cnt]=wczytaj();
		if(cnt>15000100)	while(1)	que.push(Point(1,1));
	}
    for(int i=1;i<=cnt-k;i++)    que.push(Point(sto[i],i));
    int las=0;
    for(int i=1;i<=k;i++)
    {
        Point cur;
        ...
        {
            cur=que.top();que.pop();
            if(cur.pla>las)  break;
        }
        wypisz(cur.num);
        las=cur.pla;
        que.push(Point(sto[cnt-k+i],cnt-k+i));
    }
}
