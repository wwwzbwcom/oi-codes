...
void dfs(int con)
{
	...
	else
	{
		...
		if(flag==0&&cnt%2==0)
		{
			for(int i=con;i<=end;i++)	fyp[i]=1e9,fvi[i]=0;
		//	memset(fvi,0,sizeof fvi);
			fyp[con]=0;
			priority_queue <int> que;
			que.push(-con);	vis[con]=1;
			while(!que.empty())
			{
				int i=-que.top();	que.pop();	//vis[i]=0;
				tim++;
				if()
			//	cout<<i<<"="<<fyp[i]<<endl;
				if(fyp[i]<fyp[end])
					for(int u=1;u<=n;u++)	if(mar[u]==1)
						for(int v=1;v<=n;v++)	if(mar[v]==1&&(i/mul[u])%3==1||(i/mul[v])%3==1)
						{
							int tmp=i;
							...
							if(tmp<=end)
							{
							//	cout<<i<<" "<<fyp[i]<<" "<<tmp<<" "<<dis[u][v]<<endl;
								fyp[tmp]=min(fyp[tmp],fyp[i]+dis[u][v]);
								if(tmp>i&&fvi[tmp]==0)	que.push(-tmp),fvi[tmp]=1;
							}
						}
			}
		...
		for(int u=1;u<=n;u++)	if(mar[u]==1&&(con/mul[u])%3==0)
		{
			...
			if(flag==1)
			{
				if(tmp<=end)	dfs(tmp),dyp[con]=min(dyp[con],dyp[tmp]); 
			}
			else
			{
				for(int v=1;v<=n;v++)	if(mar[v]==1&&(con/mul[v])%3)
				{
					int tmp=con;
					if((tmp/mul[u])%3<2)	tmp+=mul[u];	else	tmp-=mul[u];
					if((tmp/mul[v])%3<2)	tmp+=mul[v];	else	tmp-=mul[v];
					if(tmp<=end&&dis[u][v]<dyp[con])	dfs(tmp),dyp[con]=min(dyp[con],dyp[tmp]+dis[u][v]);
				}
			}
		}
	}
